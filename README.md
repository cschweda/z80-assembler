...
- 🚀 **Fast Assembly**: Average 0.30ms per program compilation
- 📝 **Source Editor**: 8,000 character limit with live counter and visual feedback
...
- **Source Code Editor**: 
  - 8,000 character limit (sufficient for complex programs like the poker hand analyzer)
  - Live character counter showing current usage
  - Visual feedback: normal (green), warning at 80% (yellow), danger at 95% (red)
  - Automatic paste truncation if content exceeds limit
  - Keyboard shortcut: Ctrl+Enter to assemble
- **Example Program Library**: 27 pre-written example programs covering all Z80 instruction categories
- **TRS-80 Authentic Display**: Green-on-black terminal aesthetic matching the original hardware
...
The project includes TRS-80 Model III ROM files in the `roms/` directory:
- `model3.rom` - Binary ROM image (14KB)
- `model3.rom.hex` - Human-readable hex dump
- `model3.rom.ihex` - Intel HEX format

**Note**: These files are excluded from version control (`.gitignore`) but are required for the build process. They are automatically copied to `dist/roms/` during the build for deployment. If you're cloning the repository, you'll need to obtain these files separately or they will be generated from `trs80emu.js` if present.
...
**Note**: The build process automatically copies ROM files from `roms/` to `dist/roms/` for deployment. These files are included in `.gitignore` and must be present locally for the build to complete successfully.
...
The test suite validates all 27 example programs with detailed output including:
...
1. **Select an Example**: Use the dropdown to load one of 27 example programs
2. **Edit Source**: Modify the assembly source code in the editor
   - **Character Limit**: 8,000 characters maximum (counter shown in header)
   - **Visual Feedback**: Counter turns yellow at 80% and red at 95% capacity
   - **Paste Protection**: Large pastes are automatically truncated to fit
...
- ✅ **Confidence**: 27/27 passing = assembler works correctly
...
- **ROM Files**: Automatically copied to `dist/roms/` during build (binary, hex dump, and Intel HEX formats)
  - These files are used by TRS-80 emulators and are provided in multiple formats for compatibility
  - ROM files are cached with 1-year immutable headers for optimal performance
...
The assembler includes a comprehensive automated test suite that validates all 27 example programs:
...
```
=================================================================
Running Z80 Assembler Test Suite (27 tests)
=================================================================
...
Failed:          0
Success Rate:    100%
Average Time:    0.30ms per test
Total Bytes:     640 bytes generated
Total Duration:  8.06ms
=================================================================
```
...
│   ├── examples/
│   │   └── programs.js       # 27 example programs
│   └── tests/
│       └── test-suite.js     # Comprehensive test suite
├── styles/
│   └── main.css              # TRS-80 themed styling
├── roms/                     # TRS-80 Model III ROM files (gitignored)
│   ├── model3.rom            # Binary ROM image
│   ├── model3.rom.hex        # Hex dump format
│   └── model3.rom.ihex       # Intel HEX format
├── docs/
